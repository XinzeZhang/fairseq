...
def create_bpe_dictionary(name):
    global_counter = Counter()
    ...
    combined_counter = global_counter
    counter_common = combined_counter.most_common()
    vocab_count= 100.0 * sum([count for word, count in counter_common])
    total_counts = sum(combined_counter.values())
    coverate=vocab_count/total_counts
    print()
    print(' total word: {} \n vocab word: {} \n coverate rate: {}%'.format(total_counts, vocab_count, coverate))

    # fv = open(bpeVocab)
    bpe_vocab = countword(name)
    bpe_vocab_count = []
    for word in bpe_vocab:
        count = bpe_vocab[word]
        bpe_vocab_count.append((word, count))

    vocab_count = 100.0 * sum([count for word, count in bpe_vocab_count])
    coverate = vocab_count / total_counts

    print('-'*10+'BPE' + '-'*10+' \n total word: {} \n vocab word: {} \n coverate rate: {}%'.format(
        total_counts, vocab_count, coverate))
...
def parseargs():
    ...
    msg = "corpus"
    parser.add_argument(
        "--corpus", default='./experiment/ldc/oracle.bpe/train.en', help=msg)
    msg = "output"
    parser.add_argument(
        "--output", default='experiment/ldc/oracle.bpe/train.en.pkl', help=msg)
...
if __name__ == "__main__":
    ...
    if args.groundhog:
        ...
        if args.bpe:
            vocab = create_bpe_dictionary(args.corpus)
        else:
